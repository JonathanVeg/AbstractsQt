...
void ConfNetwork::testConnectionInput()
{
    ...
    if(cbTypeConection->currentIndex() == 2)
    {
        ...
        {
            QMessageBox::warning(this, tr("Diretório do Banco"), tr("Informe corretamente o diretório do Postgres."), QMessageBox::Ok);
            return;
        }
...
void ConfNetwork::onClick_pbConnect()
{
    if(fieldVerification())
        ...
        QMessageBox::warning(this, "Dados Incorreto", "Preencha corretamente o campos.");
}
...
bool ConfNetwork::configPostgres(QString filename)
{
    ...
    QFile file(filename + "/9.1/data/pg_hba.conf"); // Destino
